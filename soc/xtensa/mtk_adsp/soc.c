...
static void enable_mpu(void)
{
	 ...
	 * disabled!
	 */
	int32_t nseg = sizeof(mpu)/sizeof(mpu[0]);
	for (int32_t i = 31; i >= 32 - nseg; i--) {
		 ...
		 *
		 * Also note: Zephyr SDK toolchains don't have MPU
		 * support, but the instruction format is identical to
		 * the data TLB instructions for RPO or MMU, so use
		 * that.
		 */
...
void c_boot(void)
{
	...
	/* The CPU clock is not affected (!) by reset from the host,
	 * so we don't know the speed we're at (on MT8195, hardware
	 * powers up at 26 Mhz, but SOF sets it to 720 and leaves it
	 * there).  Choose the highest speed unconditionally.
	 */
	mtk_adsp_set_cpu_freq(DT_PROP_LEN(DT_NODELABEL(cpuclk), freqs_mhz) - 1);
