		    ...
		    arch_cpustart_t fn, void *arg)
{
	 ...
	 *
	 * FIXME: per docs this bit in IETC is purely informative
	 * (it's a shadow of the high bit of ITC/TFC), and on 2.5
	 * hardware it always reflects the BUSY bit already cleared on
	 * the other core, making this a noop.  Seems likely to be
	 * true elsewhere, test and remove.
	 */
	uint32_t idc_reg = idc_read(IPC_IDCIETC(cpu_num), 0) | IPC_IDCIETC_DONE;
...
void soc_idc_init(void)
{
	...
	for (int core = 0; core < CONFIG_MP_NUM_CPUS; core++) {
		 ...
		 * L2 mask register.
		 */
		CAVS_INTCTRL->l2.clear = CAVS_L2_IDC;
...
int soc_halt_cpu(int id)
{
	 ...
	 */
	CAVS_INTCTRL->l2.set = CAVS_L2_IDC;
	shim->pwrctl &= ~BIT(id);
