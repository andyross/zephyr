...
void clock_calibrate(void)
{
	...
	cyc0 = k_cycle_get_32();
	inotice_send_message(INOTICE_DEV, IPCCMD_TIMESTAMP, 0);

	k_msleep(1000);
	host_dt = 0;
	inotice_set_message_handler(INOTICE_DEV, clock_msg, (void *)&host_dt);

	/* Now do it again, but with a handler to catch the result */
	cyc1 = k_cycle_get_32();
	inotice_send_message(INOTICE_DEV, IPCCMD_TIMESTAMP, 0);
	WAIT_FOR(host_dt != 0);
	inotice_set_message_handler(INOTICE_DEV, NULL, NULL);
...
void test_main(void)
{
	ztest_test_suite(intel_adsp,
			 ...
			 ztest_unit_test(clock_calibrate)
			 );
