...
static void hpet_isr(void *arg)
{
	...
	if (!IS_ENABLED(CONFIG_TICKLESS_KERNEL)) {
		TIMER0_COMPARATOR_REG = now + cyc_per_tick;
	}
...
void z_clock_set_timeout(s32_t ticks, bool idle)
{
	...
	/* Interpret "forever" as max (the HPET doesn't shut off), fix
	 * convention (arguments of 1 or less mean "at the next
	 * tick"), and clamp to a valid range.
	 */
	ticks = ticks == K_FOREVER ? max_ticks : ticks;
	ticks = max(min(ticks - 1, max_ticks), 0);
