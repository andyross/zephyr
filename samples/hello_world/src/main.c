...
/* Define this to have the cores spinning in low priority threads,
 * otherwise they're sleeping
 */
/* #define LOAD_CPUS */

/* Hardware CCOUNT rate, 400 MHz for TGL */
#define CCOUNT_HZ 400000000

/* How often to report stats */
...
void work_fn(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		while(ccount() - c0 < (CCOUNT_HZ/1000)*REPORT_MS) {}
#else
		k_msleep(REPORT_MS);
#endif
		...
		if (cpu == 0) {
			...
			for (int i = 0; i < CONFIG_MP_NUM_CPUS; i++) {
				printk("work%d: last %5d peak %6d avg %5d (N = %d)\n",
				       i, last_err[i], worst_err[i],
				       (int)(err_sum[i]/err_count[i]),
				       err_count[i]);

				/* Clear the error count each iteration, so we're
				 * tracking a local peak over the period
				 */
				worst_err[i] = 0;
			}
			printk("\n");
...
void timer_thread_fn(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		err_sum[cpu] += err;
		err_count[cpu] += 1;
		if (err_count[cpu] > 1000 && abs(err) > abs(worst_err[cpu])) {
			worst_err[cpu] = err;
		}
		last_cyc = cyc;
	}
...
{
#ifdef LOAD_CPUS
	printk("Initializing.  Worker threads will be SPINNING\n");
#else
	printk("Initializing.  Worker threads will be SLEEPING\n");
#endif
